...
#include "Lumen/Internal/API/Vulkan/VulkanRenderer.hpp"
#include "Lumen/Internal/API/Vulkan/VulkanContext.hpp"
#include "Lumen/Internal/API/Vulkan/VulkanImage.hpp"
...
namespace Lumen::Internal
{
    ...
    namespace
    {
		...
		VkPipelineBindPoint RenderpassUsageToVkPipelineBindPoint(RenderpassUsage usage)
		{
			switch (usage)
			{
            case RenderpassUsage::Graphics:             return VK_PIPELINE_BIND_POINT_GRAPHICS;
            case RenderpassUsage::Offscreen:            return VK_PIPELINE_BIND_POINT_GRAPHICS;
			case RenderpassUsage::ComputeToTexture:     return VK_PIPELINE_BIND_POINT_COMPUTE;
			}

			LU_ASSERT(false, "[VkRenderpass] Unknown RenderpassUsage passed in.");
    ...
    void VulkanRenderpass::Init(const RendererID renderer, const RenderpassSpecification& specs, CommandBuffer* commandBuffer)
    {
        ...
        LU_ASSERT(((!m_Specification.ColourAttachment.empty()) || !m_Specification.DepthAttachment.empty()), "[VkRenderpass] No Colour or Depth image passed in.");
		LU_ASSERT(!m_Specification.Subpasses.empty(), "[VkRenderpass] No subpass targets set. Most used is Colour | Depth.");
    ...
    Vec2<uint32_t> VulkanRenderpass::GetSize() const
    {
        ...
        else if (!m_Specification.DepthAttachment.empty())
        {
            size.x = m_Specification.DepthAttachment[0]->GetSpecification().Width;
            size.y = m_Specification.DepthAttachment[0]->GetSpecification().Height;
        }
    ...
    void VulkanRenderpass::CreateRenderpass(const RendererID)
    {
        ...
        if (!m_Specification.DepthAttachment.empty())
        {
            VkAttachmentDescription& depthAttachment = attachments.emplace_back();
            depthAttachment.format = ImageFormatToVkFormat(m_Specification.DepthAttachment[0]->GetSpecification().Format);
		...
		std::vector<VkSubpassDescription> subpasses(m_Specification.Subpasses.size());
        for (size_t i = 0; i < subpasses.size(); i++)
        {
			RenderpassTarget& target = m_Specification.Subpasses[i];
			
            VkSubpassDescription& subpass = subpasses[i];
			subpass.pipelineBindPoint = RenderpassUsageToVkPipelineBindPoint(m_Specification.Usage);
            
            if (target & RenderpassTarget::Colour)
            {
                LU_ASSERT(!m_Specification.ColourAttachment.empty(), std::format("[VkRenderpass] Subpass {0} requested to render to Colour attachment, but no Colour attachment was passed in.", i));
			...
			if (target & RenderpassTarget::Depth)
			{
				LU_ASSERT(!m_Specification.DepthAttachment.empty(), std::format("[VkRenderpass] Subpass {0} requested to render to Depth attachment, but no Depth attachment was passed in.", i));
                if (!m_Specification.ColourAttachment.empty())
        ...
        std::vector<VkSubpassDependency> dependencies = GetDependencies(m_Specification.Usage, m_Specification.Subpasses);
    ...
    void VulkanRenderpass::CreateFramebuffers(const RendererID renderer, uint32_t width, uint32_t height)
    {
        ...
        for (size_t i = 0; i < m_Framebuffers.size(); i++)
        {
            ...
            if (!m_Specification.DepthAttachment.empty())
            {
                if (m_Specification.DepthAttachment.size() == 1)
                {
                    VulkanImage& vkImage = m_Specification.DepthAttachment[0]->GetInternalImage();
                    attachments.push_back(vkImage.GetVkImageView());
                }
                else // If the size is not equal to 1 it has to be equal to the amount of swapchain images
                {
                    VulkanImage& vkImage = m_Specification.DepthAttachment[i]->GetInternalImage();
                    attachments.push_back(vkImage.GetVkImageView());
                }
            }
    ...
    std::vector<VkSubpassDependency> VulkanRenderpass::GetDependencies(RenderpassUsage usage, const std::vector<RenderpassTarget>& subpasses)
    {
        switch (usage)
        {
        case RenderpassUsage::Forward:
        {
            ...
            {
                std::vector<VkSubpassDependency> dependencies(subpasses.size() + 2); // +2 for external->0 and self-dependency
                ...
                // Now subpass-to-subpass dependencies start from index 2
                for (size_t i = 1; i < subpasses.size(); i++)
                {
                    dependencies[i + 1].srcSubpass = static_cast<uint32_t>(i) - 1;
                    dependencies[i + 1].dstSubpass = static_cast<uint32_t>(i);
                    dependencies[i + 1].srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
                    dependencies[i + 1].dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
                    dependencies[i + 1].srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
                    dependencies[i + 1].dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
                    dependencies[i + 1].dependencyFlags = VK_DEPENDENCY_BY_REGION_BIT;
                }

                // Last subpass -> External
                dependencies.back().srcSubpass = static_cast<uint32_t>(subpasses.size()) - 1;
                ...
                dependencies.back().dependencyFlags = 0;
