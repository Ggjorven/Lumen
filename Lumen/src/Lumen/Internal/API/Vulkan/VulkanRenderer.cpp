...
namespace Lumen::Internal
{
    ...
    void VulkanRenderer::Begin(Renderpass& renderpass)
    {
        ...
        if (!vkRenderpass.m_Specification.DepthAttachment.empty())
        {
            VkClearValue depthClear = { { { vkRenderpass.m_Specification.DepthClearColour, 0 } } };
    ...
    void VulkanRenderer::Submit(CommandBuffer& cmdBuf, ExecutionPolicy policy, Queue queue, PipelineStage waitStage, const std::vector<CommandBuffer*>& waitOn)
    {
        ...
        if (!(policy & ExecutionPolicy::NoWaiting))
        {
            ...
            if (semaphore)
            {
                ...
                #if defined(LU_CONFIG_DEBUG) // Check if semaphore not already exists
    ...
    void VulkanRenderer::NextSubpass(CommandBuffer& cmdBuf)
    {
        LU_PROFILE("VkRenderer::NextSubpass(CommandBuffer)");
        VulkanCommandBuffer& vkCmdBuf = cmdBuf.GetInternalCommandBuffer();

        {
            LU_PROFILE("VkRenderer::NextSubpass::NextSubpass");
            vkCmdNextSubpass(vkCmdBuf.m_CommandBuffers[m_SwapChain.GetCurrentFrame()], VK_SUBPASS_CONTENTS_INLINE);
        }
    }

    void VulkanRenderer::NextSubpass(Renderpass& renderpass)
    {
        LU_PROFILE("VkRenderer::NextSubpass(Renderpass)");
        NextSubpass(renderpass.GetCommandBuffer());
    }
