...
#include "Lumen/Internal/Memory/Memory.hpp"

#include "Lumen/Internal/Renderer/Buffers.hpp"
#include "Lumen/Internal/Renderer/Renderer.hpp"

#include "Lumen/Internal/API/Vulkan/VulkanAllocator.hpp"
...
namespace Lumen::Internal
{
	...
	{
		LU_PROFILE("VkVertexBuffer::SetData()");
		...
		(mappedData, data, size); // Copy only 'size' bytes
	...
	{
		LU_PROFILE("VkIndexBuffer::SetData()");
		VkBuffer stagingBuffer = VK_NULL_HANDLE;
		VmaAllocation stagingBufferAllocation = VK_NULL_HANDLE;
		stagingBufferAllocation = VulkanAllocator::AllocateBuffer(renderer, size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY, stagingBuffer, VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);

		void* mappedData = nullptr;
		VulkanAllocator::MapMemory(stagingBufferAllocation, mappedData);
		Memory::Copy(mappedData, indices, size); // Copy only 'size' bytes
		VulkanAllocator::UnMapMemory(stagingBufferAllocation);
	...
	void VulkanUniformBuffer::Init(const RendererID renderer, const BufferSpecification& specs, size_t dataSize)
	{
		m_Size = dataSize;
			...
			{
				// TODO: Make SetData use a staging buffer instead?
	...
	{
		LU_PROFILE("VkUniformBuffer::SetData()");
		...
		for (size_t i = 0; i < framesInFlight; i++)
		{
			...
			Memory::Copy(static_cast<uint8_t*>(mappedMemory) + offset, data, size);
	...
	{
		LU_PROFILE("VkStorageBuffer::SetData()");
